...
function getPageIdFromURL(url) {
    ...
    const __SP = url.match(/sp\.php/);              // Ansicht "Spielerprofil"
...
function showSpieler(zat) {
    let table1 = document.getElementById("a").getElementsByTagName("table")[0]; // neben dem Bild
    let table2 = document.getElementById("a").getElementsByTagName("table")[1]; // Skills
    let alter = stringToNumber(table1.rows[0].cells[5].textContent);
    let gebtag = stringToNumber(table1.rows[1].cells[4].textContent);
    ...
    (async () => {
        try {
            ...
            switch (getPageIdFromURL(window.location.href, {
                                                               ...
                                                               'sp.php'    : 1   // Ansicht "Spielerprofil"
                                                           }, 'page')) {
                case 0  : await procHaupt().catch(defaultCatch); break;
                case 1  : await procSpieler().catch(defaultCatch); break;
                default : break;
            }
